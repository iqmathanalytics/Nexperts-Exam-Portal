...
export const env = {
  ...
  brevoSenderName: process.env.BREVO_SENDER_NAME ?? "Ventrix Global",
  adminEmail: process.env.ADMIN_EMAIL ?? "admin@ventrix.global",
  adminPassword: process.env.ADMIN_PASSWORD ?? "Admin@Ventrix2026",
  ...
  invoiceCompanyName: process.env.INVOICE_COMPANY_NAME ?? "Ventrix Global",
  invoiceCompanyLegalName: process.env.INVOICE_COMPANY_LEGAL_NAME ?? "Ventrix Global Sdn. Bhd.",
  invoiceCompanyAddress:
    process.env.INVOICE_COMPANY_ADDRESS ??
    "Level 10, Menara XYZ|Jalan Ampang|50450 Kuala Lumpur|Malaysia",
  invoiceCompanyEmail: process.env.INVOICE_COMPANY_EMAIL ?? "billing@ventrix.global",
  invoiceCompanyPhone: process.env.INVOICE_COMPANY_PHONE ?? "+60 3-1234 5678",
  invoiceCompanyWebsite: process.env.INVOICE_COMPANY_WEBSITE ?? "https://www.ventrix.global",
